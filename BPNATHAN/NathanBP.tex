...
De installatie start op dezelfde manier als de voorgaande machine, maar in dit geval wordt de machine "`WebServer"' genoemd. Deze server wordt ook lid gemaakt van het domein "`Baele.be"'. Op deze server is het belangrijk dat de rol "`Internet Information Service 8"' (IIS8) is geïnstalleerd. Verder is de installatie van databanksoftware ook nodig. In dit geval wordt er gebruik gemaakt van Microsoft SQL Server 2014 en wordt er een testdatabase aangemaakt met de naam "`TestDatabase"'. Dan wordt er op het internet een basis ASP.net applicatie gezocht en gedownload om deze dan onder de naam van "`baele.be"' in IIS 8 te importeren. Als er in een webbrowser dan voorgaand adres wordt ingetyped, dan zou de ASP.net applicatie zichtbaar moeten zijn. 
