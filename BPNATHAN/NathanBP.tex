...
\section{Metasploit telnet + putty}
Telnet wordt in het algemeen gezien als onveilig en wordt daarom niet zoveel meer gebruikt. De variant van telnet, genaamd ssh, wordt veel meer gebruikt. Dit voorbeeld kan ook toegepast worden op dezelfde manier bij ssh, het enige verschil is dat het poortnummer van ssh nummer 22 is i.p.v. 23 voor telnet. 
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.60]{img/Metasploit_PortScan}
\end{center}
\caption{Resultaat van een port scan van de webserver}
\label{img:PortScan}
\end{figure}

In het andere terminalvenster kan nu de metasploit console worden geopend en kan er gezocht worden naar bekende exploit voor telnet. Dit gebeurt met de volgende twee lijnen: 
\begin{verbatim} msfconsole \end{verbatim}
\begin{verbatim} search telnet \end{verbatim}
Dit geeft een hele lijst van exploits voor telnet weer, maar in dit geval is er maar één nodig en dat is \textit{telnet\_login}. Deze exploit wordt geselecteerd door het volgende in te geven:
\begin{verbatim} use auxiliary/scanner/telnet/telnet\_login \end{verbatim}
Nu is de exploit geselecteerd en kan er begonnen worden met de nodige opties in te geven om de aanval succesvol te laten uitvoeren. Er kan eens gekeken worden naar de opties door het volgende in te geven:
\begin{verbatim} show options \end{verbatim}
Er zijn talrijke opties zichtbaar maar er zijn er maar een paar die nodig zijn. Het eerste wat handig is om aan te zetten is \textit{BLANK\_PASSWORDS}. Dit zorgt ervoor dat er eens wordt geprobeerd om in te loggen zonder wachtwoord, dit zal zelden lukken maar het kan gebeuren. Het volgende dat zeker moet ingevuld worden is de \textit{RHOSTS}-optie. Hier wordt het IP-adres ingegeven van het slachtoffer, de webserver die eerder is onderzocht door een portscan. Het volgende dat kan worden ingevuld is de optie \textit{USERNAME}. Dit kan het beste worden ingesteld als \textit{Administrator} omdat dit de defaultnaam is. Tot slot moet er een wachtwoordbestand meegegeven worden met de optie \textit{PASS\_FILE}. Dit is een txt-bestand waarin er allemaal verschillende wachtwoordcombinaties staan en deze zullen allemaal één voor één utigeprobeerd worden. Dit kan allemaal gedaan worden met de volgende lijnen code:
\begin{verbatim} set BLANK\_PASSWORDS true \end{verbatim}
\begin{verbatim} set RHOSTS 192.168.1.3 \end{verbatim}
\begin{verbatim} set USERNAME Administrator \end{verbatim}
\begin{verbatim} set PASS\_FILE /root/Desktop/pass.txt \end{verbatim}
